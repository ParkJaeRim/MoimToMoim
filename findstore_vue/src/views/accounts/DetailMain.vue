            ...
            <v-col cols="3" class="pt-1 pl-0 yb-0">
              ...
              <v-dialog v-model="dialog" calss>
                ...
                <v-card>
                  ...
                  <!-- 1. 완료 와 다음 버튼에 v-if 를 먹여서 reslist.length 를 사용해서 구분해줘야함
                 2. reslist.length 를 이용하여 y++ 해주는것에 제한을 둬야함  1,2번이 같은말임 if y==reslist.length 같으면 완료 아니면 다음
                 2-1. 가게별 name 과 img 가져와서 띄워주고, 별점 vuetify 이용해서 만들고, text-field 넣어서 리뷰 데이터 받아야됨.
                 3. 다음에서 method : reviewdata 가 reviews 로 push 해줘야함. 정보까지 같이 넣어줘야됨 methondname(reviewdata)
                 3-1. 이거 method 만들어서 넣어 줄 때 reviewdata 들 중에서 res_id, res_name, user_name(이건 m_id임), 내가 썼던 rating 과 review 도 넣어줌
                 3-2. 마지막 push 해주고 나서 마지막에는 깡통으로 만들어줘야됨 (reviewdata 를)
                 4. 완료 되었을 때는 다른 method 를 써서 api - views 에 review create 를 사용하여 DB에 넣어준다.
                 4-1. 3번 method 한번 써주고 4번 진행
                -->
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn
                    ...
                    <v-btn
                      ...
                      color="purple lighten-1"
                      text
                      @click="pushReviewData(item.reslist[y],item.meeting.id);y++;"
                      >다음</v-btn
                    >
                    <v-btn
                      v-else
